...
xTaskHandle TaskHandle_1; //nabto
xTaskHandle TaskHandle_2; //mqtt
xTaskHandle TaskHandle_3; //button ON/OFF
...
#define TEST_DEVICE_NAME            "BLE_XLAMP"
...
void button_task(void *pvParameter)
{
    ...
    */
	...
	//Reduce the latency for button ON/OFF
    while(1) {
		...
		val = gpio_get_level(BTN_GPIO);
		if (val == 0 && val_old == 1)
		{
			lamp = !lamp;
			gpio_set_level(LED_GPIO, lamp);
		}
		val_old = val;
		vTaskDelay(100 / portTICK_PERIOD_MS);
    }

	//important: avoid stackoverflow
...
static esp_err_t mqtt_event_handler_cb(esp_mqtt_event_handle_t event)
{
    ...
    switch (event->event_id) {
        case MQTT_EVENT_CONNECTED:
            ...
            msg_id = esp_mqtt_client_publish(client, "/magiclamp/qos1", "data_3", 0, 1, 0);
            ESP_LOGI(TAG, "sent publish successful, msg_id=%d", msg_id);

            msg_id = esp_mqtt_client_subscribe(client, "/magiclamp/qos0", 0);
            ESP_LOGI(TAG, "sent subscribe successful, msg_id=%d", msg_id);

            msg_id = esp_mqtt_client_subscribe(client, "/magiclamp/qos1", 1);
            ESP_LOGI(TAG, "sent subscribe successful, msg_id=%d", msg_id);

            msg_id = esp_mqtt_client_unsubscribe(client, "/magiclamp/qos1");
        ...
        case MQTT_EVENT_SUBSCRIBED:
            ...
            msg_id = esp_mqtt_client_publish(client, "/magiclamp/qos0", "data", 0, 0, 0);
            ESP_LOGI(TAG, "sent publish successful, msg_id=%d", msg_id);
            break;
...
static void mqtt_app_start(void *pvParameter)
{
    esp_mqtt_client_config_t mqtt_cfg = {
        .uri = "mqtt://27.71.228.66:1883" //CONFIG_BROKER_URL,
    };
...
void esp_nvs_write(char *ssid,char  *pw);
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_WRITE_EVT: {
        ...
        if (!param->write.is_prep){
            ...
            ESP_LOGI(GATTS_TAG,"data 2 %s",param->write.value);

			//NVS Write
			char ssid_temp[20],pw_temp[20];
			sscanf((char*)param->write.value,"%s - %s",ssid_temp,pw_temp);
			esp_nvs_write(ssid_temp,pw_temp);
...
static void gatts_profile_b_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_WRITE_EVT: {
        ...
        if (!param->write.is_prep){
            ESP_LOGI(GATTS_TAG, "GATT_WRITE_EVT, value len %d, value :", param->write.len);
            esp_log_buffer_hex(GATTS_TAG, param->write.value, param->write.len);
            ESP_LOGI(GATTS_TAG,"data 1 %s",param->write.value);
...
//NVS read/write
void esp_nvs_read(char *ssid,char  *pw)
{
	esp_err_t err;
	...
	lenght = 32;
	err = nvs_get_str(my_handle,"ssid",ssid,&lenght);
    switch (err) {
        case ESP_OK:
            ESP_LOGI(GATTS_TAG,"Done %s \n",ssid);
            break;
        case ESP_ERR_NVS_NOT_FOUND:
            ESP_LOGE(GATTS_TAG,"The value is not initialized yet!\n");
			memcpy(ssid,"xcomp",5);
            break;
        default :
            ESP_LOGE(GATTS_TAG,"Error (%s) reading!\n", esp_err_to_name(err));
    }
	lenght = 64;
	err = nvs_get_str(my_handle,"pw",pw,&lenght);
    switch (err) {
        case ESP_OK:
            ESP_LOGI(GATTS_TAG,"Done %s \n",pw);
            break;
        case ESP_ERR_NVS_NOT_FOUND:
            ESP_LOGE(GATTS_TAG,"The value (%s) is not initialized yet!\n", esp_err_to_name(err));
			memcpy(pw,"1111111111",10);
            break;
        default :
            ESP_LOGE(GATTS_TAG,"Error (%s) reading!\n", esp_err_to_name(err));
    }
...
void esp_nvs_write(char *ssid,char  *pw)
{
    ...
    //ESP_LOGI(GATTS_TAG,((err != ESP_OK) ? "Failed!\n" : "Done\n"));
	ESP_ERROR_CHECK(err);

	err = nvs_set_str(my_handle,"pw", pw);
    //ESP_LOGI(GATTS_TAG,((err != ESP_OK) ? "Failed!\n" : "Done\n"));
	ESP_ERROR_CHECK(err);
...
void esp_bt_ble_start(void *pvParameter)
{
	vTaskDelay(200 / portTICK_PERIOD_MS);
	...
	//free resources and delete task
	...
	//Keep looping to force ble controller to wait for wifi task in main loop
exit :
	while(1) {
...
void app_main()
{
	...
	esp_err_t ret;
	//char ssid_nvs[20],password_nvs[20];
  ...
  */
  ...
  /*wifi_config_t wifi_config = {
    .sta = {
      .ssid = (char*) ssid_nvs,
      .password = (char*) password_nvs,
    },
  };*/

	//NVS read
	wifi_config_t wifi_config;
	esp_nvs_read((char *)wifi_config.sta.ssid,(char *)wifi_config.sta.password);
  ...
  // start the main task
  xTaskCreate(&main_task, "main_task", 8192, NULL, 1, &TaskHandle_1);
  ...
  // Go do something else here
	...
	//disable example_connect() func that blocks until connection established and IP address obtained
	...
	//vTaskDelay(10 / portTICK_PERIOD_MS);
	//ESP32-MQTT IMPL
    //diable mqtt_app_start for good!
	//xTaskCreate(&mqtt_app_start, "mqtt_app_start", 8192, NULL, 2, &TaskHandle_2);

    NABTO_LOG_ERROR(("MQTT Init Success "));


	//Bluetooth-BLE
	xTaskCreate(&esp_bt_ble_start, "esp_bt_ble_start", 8192, NULL, 3, NULL);

	//added button_task func 06/05
